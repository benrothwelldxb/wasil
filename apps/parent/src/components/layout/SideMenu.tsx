...
export function SideMenu({ open, onClose }: SideMenuProps) {
  ...
  // Shown when the school has the module on — the flag is the switch, and the
  // route 404s without it either way. Deliberately NOT gated on whether the
  // child has anything this week: a parent whose week is quiet still needs a
  // way in to see that it is.
  if (isEnabled('activeScheduleEnabled')) {
  ...
  return (
    ...
    {/* iOS "Add to Home Screen" instructions — shown when there's no native
        install prompt (iPhone/iPad, where Safari has no install API). Same
        centred dialog the post-login nudge uses, so both read identically. */}
    {showIosInstall && (
      <InstallDialog
        canPrompt={false}
        onInstall={() => setShowIosInstall(false)}
