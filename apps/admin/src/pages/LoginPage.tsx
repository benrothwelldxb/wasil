...
export function LoginPage() {
  ...
  const handleDemoLogin = async (type: 'admin' | 'superadmin' | 'staff') => {
    setError(null)
    try {
      if (type === 'admin') {
        await login('admin@vhps.ae', 'ADMIN')
      } else if (type === 'superadmin') {
        await login('superadmin@wasil.app', 'SUPER_ADMIN')
      } else {
        await login('staff@vhps.ae', 'STAFF')
      }
    } catch (err) {
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full">
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            style={{ backgroundColor: theme.colors.brandColor }}
          >
            {isLoading ? 'Signing in...' : 'Demo: Admin'}
          </button>

          <button
            onClick={() => handleDemoLogin('superadmin')}
            disabled={isLoading}
            className="w-full py-2.5 rounded-lg font-medium transition-colors disabled:opacity-50 border"
            style={{ borderColor: theme.colors.brandColor, color: theme.colors.brandColor }}
          >
            {isLoading ? 'Signing in...' : 'Demo: Super Admin'}
          </button>
        </div>
