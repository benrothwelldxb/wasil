...
interface JourneyCardProps {
  ...
  rank?: number;
  /**
   * Whether this card opts into entrance/layout animation. Defaults to
   * `true`; `ResultsPage` sets this to `false` beyond `ANIMATED_CARD_LIMIT`
   * so a large result set can't turn every render into an unbounded
   * `layout`/`AnimatePresence` pass.
   */
  animated?: boolean;
}

export function JourneyCard({ journey, rank, animated = true }: JourneyCardProps) {
  ...
  const link = (
    ...
    >
    {/* Only transform/box-shadow/border-color actually animate on hover —
          `transition-all` was tracking every animatable property (including
          ones that never change), which is needless paint/layout work. */}
      <Card className="overflow-hidden p-0 transition-[transform,box-shadow,border-color] duration-200 group-hover:border-primary/40 group-hover:shadow-lg group-focus-visible:ring-2 group-focus-visible:ring-ring group-focus-visible:ring-offset-2">
