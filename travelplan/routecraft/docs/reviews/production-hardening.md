...
| Security | **FAIL** | Strict CSP (`script-src`/`style-src 'self'`, no inline — theme boot externalised to `/theme-init.js`) + `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, HSTS via `_headers`. No secrets/PII in bundle (verified). `npm audit fix` applied. |
